...
4. 클로드가 가져온 데이터를 아무 파일에다가 통째로 덮어쓰기 (예시: /Users/맥북계정/Desktop/current.json — 매월 같은 파일 사용)
5. 맥북앱 → 당직 탭 → '당직 알림 설정' 모달 → '파일에서 가져오기'에서 사용자가 만든 파일 경로를 넣어준 뒤 '적용하기' 버튼을 누른다.
   - 사람 파일 입력칸: `/Users/맥북계정/Desktop/people.json` (예시)
   - 월별 배정 입력칸: `/Users/맥북계정/Desktop/current.json` (예시)
6. 다음날 09시(기본)에 슬랙 당직 알림 자동 발송
7. (선택) 조회 전용 비공개 next.js 파일도 `git push` → Vercel이 비공개 프로젝트를 알아서 배포 → 팀원도 웹에서 조회 가능

> **실제 사용자 환경의 경로/위키 URL 등은 같은 폴더의 `SYNC_LOCAL.md` 참조.**
> 본인(레포 소유자) 전용 메모로, gitignore로 빠져있어 커밋되지 않는다.
> 다음 클로드는 동기화 작업 시작 시 `SYNC_LOCAL.md`를 먼저 읽어 본인 환경의 실제 파일 경로를 파악할 것.
